...
"SELECT title, SUM(amount) FROM pledges LEFT JOIN projects ON pledges.project_id = projects.id GROUP BY projects.id ORDER BY projects.title;"
end
...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amt_over_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY project_id HAVING amt_over_goal >= 0;"
end
